	...
	if (cashPull < 100 || cashPull >150000 || cashPull % 100 != 0 ) {
		...
		if (cashPull >= 5000) {
		  int cashOnHand = cashPull / 5000;
			std::cout << cashOnHand << " по 5000р, ";
			cashPull -= (cashOnHand * 5000);
		}

		if (cashPull >= 2000) {
				int cashOnHand = cashPull / 2000;
				std::cout << cashOnHand << " по 2000р, ";
				cashPull -= (cashOnHand * 2000);
				}

		if (cashPull >= 1000) {
				int cashOnHand = cashPull / 1000;
				std::cout << cashOnHand << " по 1000р, ";
				cashPull -= (cashOnHand * 1000);
				}

		if (cashPull >= 500) {
				int cashOnHand = cashPull / 500;
				std::cout << cashOnHand << " по 500р, ";
				cashPull -= (cashOnHand * 500);
				}

		if (cashPull >= 200) {
				int cashOnHand = cashPull / 200;
				std::cout << cashOnHand << " по 200р, ";
				cashPull -= (cashOnHand * 200);
				}

		if (cashPull >= 100) {
				int cashOnHand = cashPull / 100;
				std::cout << cashOnHand << " по 100р, ";
				cashPull -= (cashOnHand * 100);
				}
	}
}
