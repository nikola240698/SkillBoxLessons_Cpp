...
int main() {
    ...
    int barberSpeed     = 1;        //колчисетву мужчин за 1 час работы барбера

    int manNumber, barberNumberInStock, barberNumberNeed;
    int barberPowerMonth;
    int totalMonthNumber;

    std::cout << "Пограмма для вычисления необходимого колличества барберов" << std::endl;
    std::cout << std::endl;
    std::cout << "Укаэите, сколько мужчин проживает в городе? ";
    std::cin >> manNumber;
    std::cout << "Укажте, сколько барберов уже работают в городе? ";
    ...
    std::cout << "Для успешной компании требуется " << totalMonthNumber << " барберов" << std::endl;
    ...
    if (totalMonthNumber < barberNumberInStock) {
        ...
        std::cout << "Предлагаем сократить " << barberNumberInStock - totalMonthNumber << " барберов" << std::endl;
    }
